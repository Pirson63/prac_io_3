...
def get_sold_items(sell):
    # Сделаем так, чтобы для записи, в которой отражена продажа яблока,
    # в столбце scu_num стояла 1, а в столбце num был 0.
    # для карандаша 0 и 1 соответственно.
    ...
    # В колонке scu_num, отвечающей за яблоки, обнулили все 1,
    # так как они сообщают о том, что был продан карандаш,
    # и вместо -1 поставили 1.
    # Аналогично в колонке num, отвечающей за карандаши,
    # обнулили все -1, так как они сообщают о том, что было продано яблоко,
    # и оставили 1. 
...
def daily_inventory(supply, sell):
    # суммируем все произошедшие поставки и вычтем
    #общее количество проданных товаров к текущему дню.
...
def monthly_theft(daily_inv, inventory):
    ...
    # Сформировали группы,объединяющие записи о ежедневном состоянии
    # склада по месяцам, с помощью resample(’M’) и из каждой группы оставили
    # только последнюю по времени запись, используя last().
...
def update(state, sell, steal, dframe):
    # Сформируем группы, объединяющие записи о ежедневных продажах
    # по годам, с помощью resample(’Y’) и для каждой группы выполним суммирование.
...
for e in file_names:
    ...
    # Содержит агрегированные данные об объёмах продаж
